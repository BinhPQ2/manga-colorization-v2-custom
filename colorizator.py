...
class MangaColorizator:
    ...
    def colorize(self, image_to_get_ratio=None):
        ...
        # Remove padding using the undo_padding function
        result = undo_padding(result, self.current_pad)

        original_height, original_width = image_to_get_ratio.shape[0], image_to_get_ratio.shape[1]
        result = resize_image(result, (original_width, original_height))  # Resize back to original dimensions
